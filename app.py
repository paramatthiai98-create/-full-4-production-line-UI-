...
st.set_page_config(page_title="SmartSafe Factory 4-Camera Dashboard", layout="wide")
...
.grid-card {
    background: linear-gradient(180deg, rgba(13,20,37,0.96), rgba(10,16,28,0.98));
...
def estimate_distance_cm(person_box, frame_width):
    x1, _, x2, _ = person_box
    person_center_x = (x1 + x2) / 2
    machine_anchor_x = frame_width * 0.82
    person_width = max(1, x2 - x1)
    pixel_distance = abs(machine_anchor_x - person_center_x)
...
uploaded_images = st.sidebar.file_uploader(
    "Upload 4 camera images (Line 1–4)",
    type=["jpg", "jpeg", "png"],
...
st.markdown('<div class="main-title">SmartSafe Factory 4-Camera Dashboard</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-title">4 Production Lines = 4 กล้อง / 4 รูป แยกแต่ละไลน์จริง</div>', unsafe_allow_html=True)
...
if analyze_btn:
    if person_model is None:
    ...
    elif uploaded_images is None or len(uploaded_images) != 4:
        st.error("กรุณาอัปโหลดรูปให้ครบ 4 รูป สำหรับ Line 1–4")
    else:
        ...
        for i, uploaded_file in enumerate(uploaded_images):
            ...
            if frame is None:
                st.error(f"ไม่สามารถเปิดรูปของ {line} ได้")
                st.stop()
            ...
            # draw machine zone per camera
            cv2.rectangle(annotated, (int(w * 0.74), int(h * 0.12)), (int(w * 0.95), int(h * 0.88)), (255, 170, 0), 2)
            cv2.putText(annotated, f"{line} MACHINE", (int(w * 0.74), max(25, int(h * 0.10))), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 170, 0), 2)
            ...
            for person_box in person_boxes:
                ...
                distance_cm = estimate_distance_cm(person_box, w)
                ...
                line_data[line]["people"] += 1
                line_data[line]["risk_total"] += risk
                line_data[line]["reasons"].extend(reasons)

                total_people += 1
                total_risk += risk

                if helmet_status == "helmet":
                    line_data[line]["helmet"] += 1
                    total_helmet += 1
                    color = (30, 200, 80)
                    label_text = f"Helmet | {distance_cm} cm"
                elif helmet_status == "no_helmet":
                    line_data[line]["no_helmet"] += 1
                    total_no_helmet += 1
                    color = (0, 0, 255)
                    label_text = f"No Helmet | {distance_cm} cm"
                else:
                    line_data[line]["unknown"] += 1
                    total_unknown += 1
                    color = (0, 200, 255)
                    label_text = f"Person | {distance_cm} cm"
            ...
            annotated_frames[line] = annotated
        ...
        top_left, top_right = st.columns([1.7, 1.0], gap="large")

        with top_left:
            st.markdown('<div class="panel">', unsafe_allow_html=True)
            st.markdown('<div class="section-title">Factory Cameras View</div>', unsafe_allow_html=True)

            cam_row1 = st.columns(2, gap="medium")
            with cam_row1[0]:
                st.markdown('<div class="camera-label">📷 Line 1</div>', unsafe_allow_html=True)
                st.image(annotated_frames["Line 1"], channels="BGR", use_container_width=True)
            with cam_row1[1]:
                st.markdown('<div class="camera-label">📷 Line 2</div>', unsafe_allow_html=True)
                st.image(annotated_frames["Line 2"], channels="BGR", use_container_width=True)

            cam_row2 = st.columns(2, gap="medium")
            with cam_row2[0]:
                st.markdown('<div class="camera-label">📷 Line 3</div>', unsafe_allow_html=True)
                st.image(annotated_frames["Line 3"], channels="BGR", use_container_width=True)
            with cam_row2[1]:
                st.markdown('<div class="camera-label">📷 Line 4</div>', unsafe_allow_html=True)
                st.image(annotated_frames["Line 4"], channels="BGR", use_container_width=True)
        ...
        with compare1:
            compare_df = pd.DataFrame({
                ...
                "People": [
                    line_data["Line 1"]["people"],
                    line_data["Line 2"]["people"],
                    line_data["Line 3"]["people"],
                    line_data["Line 4"]["people"],
                ],
                "Helmet": [
                    line_data["Line 1"]["helmet"],
                    line_data["Line 2"]["helmet"],
                    line_data["Line 3"]["helmet"],
                    line_data["Line 4"]["helmet"],
                ],
                "No Helmet": [
                    line_data["Line 1"]["no_helmet"],
                    line_data["Line 2"]["no_helmet"],
                    line_data["Line 3"]["no_helmet"],
                    line_data["Line 4"]["no_helmet"],
                ],
                "Unknown": [
                    line_data["Line 1"]["unknown"],
                    line_data["Line 2"]["unknown"],
                    line_data["Line 3"]["unknown"],
                    line_data["Line 4"]["unknown"],
                ],
            }).set_index("Line")
